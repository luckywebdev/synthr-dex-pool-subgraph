...
import { BIG_DECIMAL_ONE, PRICE_ORACLE } from "../constants";
import { PriceOracle as PriceOracleABI } from "../../../generated/LBPairV21/PriceOracle";
...
export function getNativePriceInUSD(): BigDecimal {
  const contract = PriceOracleABI.bind(Address.fromString(PRICE_ORACLE));
  let decimal = BigInt.fromI32(8)
  const oracleDecimal = contract.try_decimals();
  if (!oracleDecimal.reverted) {
    decimal = BigInt.fromI32(oracleDecimal.value)
  }
  const latestOraclePrice = contract.try_latestAnswer();
  if (latestOraclePrice.reverted) {
    return BIG_DECIMAL_ONE
  }
  return latestOraclePrice.value.toBigDecimal().div(BigDecimal.fromString("1e" + decimal.toString()))
}
